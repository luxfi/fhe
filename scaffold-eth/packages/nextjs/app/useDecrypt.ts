...
import { fhesdkClient, useFHEAccount, useFHEConnected } from "./useFHE";
import { FheTypes, UnsealedItem } from "@fhe/sdk";
...
const _decryptValue = async <T extends FheTypes>(fheType: T, value: bigint): Promise<DecryptionResult<T>> => {
  ...
  const result = await fhesdkClient.decryptHandle(value, fheType).decrypt();
  if (result.success) {
    logBlockMessageAndEnd(
...
/**
 * Hook to decrypt a value using fhe sdk
...
export const useDecryptValue = <T extends FheTypes>(
  ...
  const fheAccount = useFHEAccount();
  const fheConnected = useFHEConnected();
  ...
  const onDecrypt = useCallback(async () => {
    ...
    if (!fheConnected || fheAccount == null) {
      setResult({
        fheType,
        ctHash,
        value: null,
        error: !fheConnected ? "FHE not connected" : "No account connected",
        state: "error",
      });
      return;
    }
  ...
  }, [fheType, ctHash, fheAccount, fheConnected]);
