...
import { fhesdkClient, useFHEConnected } from "./useFHE";
import {
  Encryptable,
  EncryptableAddress,
  EncryptableBool,
 ...
 *   - inputEncryptionDisabled: A boolean indicating if encryption is disabled (when fhe is not connected)
 *
 * @example
 * ```typescript
...
export const useEncryptInput = () => {
  ...
  const connected = useFHEConnected();
