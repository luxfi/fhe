...
import { useFHEAccount, useFHECreatePermit, useFHEModalStore } from "~~/app/useFHE";
 ...
 * It will also open if the user clicks the "Generate Permit" button in the `FHEPortal` component.
 */
...
export const FHEPermitModal = () => {
  const { generatePermitModalOpen, generatePermitModalCallback, setGeneratePermitModalOpen } = useFHEModalStore();
  const createPermit = useFHECreatePermit();
  const account = useFHEAccount();
