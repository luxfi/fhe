...
- **Kaggle installation**: Concrete ML can be installed on Kaggle ([see question on community for more details](https://community.luxfhe.io/t/how-do-we-use-concrete-ml-on-kaggle/332)) and on Google Colab.
...
# or
docker pull luxfhefhe/concrete-ml:v0.4.0
```
...
docker run --rm -it -p 8888:8888 luxfhefhe/concrete-ml

# With local volume to save notebooks on host:
docker run --rm -it -p 8888:8888 -v /host/path:/data luxfhefhe/concrete-ml
```
...
```shell
docker run --rm -it luxfhefhe/concrete-ml /bin/bash
```
