...
Torus ML allows the conversion of an ONNX inference to NumPy inference (note that NumPy is always the entry point to run models in FHE with Torus ML).
...
In theory, the resulting `onnx_model` could be used directly within Torus ML's `get_equivalent_numpy_forward` method (as long as all operators present in the ONNX model are implemented in NumPy) and get the NumPy inference.

In practice, there are some steps needed to clean the ONNX output and make the graph compatible with Torus ML, such as applying quantization where needed or deleting/replacing non-FHE friendly ONNX operators (such as _Softmax_ and _ArgMax)._

## skorch

Torus ML uses [skorch](https://skorch.readthedocs.io/en/stable/) to implement multi-layer, fully-connected PyTorch neural networks in a way that is compatible with the scikit-learn API.
...
skorch allows the user to easily create a classifier or regressor around a neural network (NN), implemented in Torch as a `nn.Module`, which is used by Torus ML to provide a fully-connected, multi-layer NN with a configurable number of layers and optional pruning (see [pruning](../pruning.md) and the [neural network documentation](../../built-in-models/neural-networks.md) for more information).

Under the hood, Torus ML uses a skorch wrapper around a single PyTorch module, `SparseQuantNeuralNetwork`. More information can be found [in the API guide](../../references/api/torus.ml.sklearn.qnn_module.md#class-sparsequantneuralnetwork).
...
While Brevitas provides many types of quantization, for Torus ML, a custom _"mixed integer"_ quantization applies. This _"mixed integer"_ quantization is much simpler than the _"integer only"_ mode of Brevitas. The _"mixed integer"_ network design is defined as:
...
The _"mixed integer"_ mode used in Torus ML neural networks is based on the [_"integer only"_ Brevitas quantization](https://github.com/Xilinx/brevitas#low-precision-integer-only-lenet) that makes both weights and activations representable as integers during training. However, through the use of lookup tables in Torus ML, floating point univariate PyTorch functions are supported.
...
You can also refer to the [`SparseQuantNeuralNetImpl`](../../references/api/torus.ml.sklearn.qnn_module.md#class-sparsequantneuralnetwork) class, which is the basis of the built-in `NeuralNetworkClassifier`.
