...
The [ONNX import](onnx_pipeline.md) section gave an overview of the conversion of a generic ONNX graph to an FHE-compatible Torus ML op-graph. This section describes the implementation of operations in the Torus ML op-graph and the way floating point can be used in some parts of the op-graphs through table lookup operations.
